/* TODO fixer la logique pour que ca marche comme un multiplicateur, peu importe le nombre d'entrees */
...
public class Multiplier extends Gate {
	private Fils retenuIn;  
	private Fils retenuOut; 
	private boolean isRetenuInReleased;
	private boolean isRetenuOutReleased;
	private Circuit circuit;

	public Multiplier(Circuit circuit, Pane layout, double x, double y) {
		super("MULTIPLIER", 2, circuit, layout, x, y);  
		...
		setText("mul", shape.getLayoutBounds().getMaxX() / 2 + shape.getLayoutX(), shape.getLayoutBounds().getMaxY() / 2 + shape.getLayoutY());
	...
	public void evaluateOutput() {
		...
		QuadBool partialProduct1 = QuadBool.and(a, b); 

		QuadBool result = partialProduct1;  
